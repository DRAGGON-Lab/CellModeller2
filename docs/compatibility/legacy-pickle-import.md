...
The resulting file is an ordinary authenticated CellModeller2 JSON checkpoint with a zero species-rate plan. It contains no executable model source. Its provenance records the input digest, legacy format, time basis, model-source digest when present, and the names of all dropped cell attributes.
