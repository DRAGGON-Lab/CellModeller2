...
| P1 | Legacy Python model adapter | module regulator | adapter for maintained `setup/init/update/divide` models | growth/mechanics callbacks, constraints, neighbors, asymmetric and alternating division, host-only species state, setup facade, and exact batch resume implemented; source-pinned 25-example CPU/Metal matrix covers 15 unchanged models, 9 typed migrations, and one pickle-migration-only example |
