...
| P0 | Native model orchestration | simulator/module lifecycle | restartable controller over native state | structural controller contract, exact explicit-source resume, runtime random streams, batch manifests, and live viewer integration implemented; reusable typed regulation/division/mechanics policies remain in progress |
| P1 | Legacy Python model adapter | module regulator | adapter for maintained `setup/init/update/divide` models | growth/mechanics callbacks, constraints, neighbors, asymmetric and alternating division, setup facade, and exact batch resume implemented; signaling models require typed-plan migration |
